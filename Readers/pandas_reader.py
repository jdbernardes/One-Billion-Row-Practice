...
class PandasReader:
    ...
    def __init__(self, chunk_size: int, total_linhas_conhecidas: int,
                 file_path: str) -> None:
    ...
    def process_chunk(self, chunk: pd.DataFrame) -> pd.DataFrame:
        aggregated = chunk.groupby('Card_Provider')['Card_Number']\
            .count().reset_index()
        return aggregated

    def create_df_with_pandas(self) -> pd.DataFrame:
        total_chunks = self._total_linhas_conhecidas // self._chunk_size + \
            (1 if self._total_linhas_conhecidas % self._chunk_size else 0)
        results = []
        total_records_processed: int = 0

        with pd.read_csv(self._file_path,
                         chunksize=self._chunk_size) as reader:
        ...
        final_aggregate = final_df.groupby('Card_Provider')['Card_Number']\
            .sum()
